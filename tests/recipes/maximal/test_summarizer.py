...
class _FakeLLM:
    """Minimal stub that mimics LangChain BaseChatModel.invoke."""
...
def test_abstract_uses_abstract_prompt_and_returns_text() -> None:
    llm = _FakeLLM("SHORT SUMMARY")
    s = _make(llm, ab="CUSTOM_SYS")
    out = s.abstract("the content")
...
def test_overview_uses_overview_prompt() -> None:
    llm = _FakeLLM("LONG OVERVIEW")
    s = _make(llm, ov="OV_SYS")
    out = s.overview(b"bytes here")
    assert out == "LONG OVERVIEW"
    sys_msg = llm.last_messages[0]
    assert sys_msg.content == "OV_SYS"


def test_abstract_decodes_bytes() -> None:
    llm = _FakeLLM("ok")
    s = _make(llm)
    s.abstract(b"\xe4\xb8\xad\xe6\x96\x87")  # UTF-8 Chinese
